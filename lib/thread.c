...
//  return buf;				//serious err in old version zebra
//}
...
struct thread* thread_add_timer_high_resolution(struct thread_master *m,
		  int (*func) (struct thread *), void *arg, unsigned long microsecends)
{
	  struct timeval timer_now;
	  struct thread *thread;
	  ...
	  gettimeofday (&timer_now, NULL);
	  timer_now.tv_usec += microsecends % TIMER_SECOND_MICRO;
	  timer_now.tv_sec += microsecends / TIMER_SECOND_MICRO +  timer_now.tv_usec / TIMER_SECOND_MICRO;
	  timer_now.tv_usec %= TIMER_SECOND_MICRO;
...
{
	return thread_add_timer_high_resolution(m, func, arg, timer * TIMER_SECOND_MICRO);
}
...
thread_cancel (struct thread *thread)
{
  switch (thread->type)
    ...
    case THREAD_READY:
      ...
      break;
    default:
      return; //notice, if THREAD_UNUSED, DO NOT add to unuse queue twice.
    }
...
thread_fetch(struct thread_master *m, struct thread *fetch)
{
	...
	if (!m->event.head && !m->read.head && !m->ready.head && !m->timer.head
			&& !m->write.head)
		return NULL;
...
{
  unsigned long thread_time;
  RUSAGE_T ru;			//just a timeval

  GETRUSAGE (&thread->ru);	//if have system support, then use struct rusage, or timeval instead.
